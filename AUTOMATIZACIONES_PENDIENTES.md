...
- **Qué**: desde que Google apagó la API legacy de FCM (mediados de 2024), el servicio de push de Expo necesita **dos cosas** de un proyecto de Firebase propio — no alcanza con una sola:
  1. `google-services.json` (config nativa) referenciado en `app.json` → `expo.android.googleServicesFile`. Sin esto, `FirebaseApp` no inicializa en el dispositivo (`"Default FirebaseApp failed to initialize because no default options were found"` en logcat) y `getExpoPushTokenAsync()` no puede ni pedir un token.
  2. La clave de cuenta de servicio (Google Service Account, FCM V1) subida a EAS vía `eas credentials`. Sin esto, el token SÍ se registra pero el envío (`rh_enviar_push()` → `exp.host/--/api/v2/push/send`) responde `"InvalidCredentials"` — Expo no tiene con qué autenticarse contra Google para entregar el mensaje.
- **Estado**: **hecho y verificado en vivo (2026-08-31)**. Se creó el proyecto "Red Huellitas" en Firebase Console, se agregó la app Android (`com.redhuellitas.app`), se bajó `google-services.json` (ahora en `app-movil/google-services.json`, gitignoreado — no es secreto grave pero es específico del proyecto) y se subió la Service Account Key vía `eas credentials -p android` → **"Google Service Account"** → "Set up a Google Service Account Key for Push Notifications (FCM V1)" (ojo: NO es la opción "Push Notifications (Legacy)", esa ya no sirve). Probado con una cuenta descartable en el S23 Ultra: el push llegó de verdad a la barra de notificaciones de Android (`NotificationRecord` real vía `dumpsys notification`, visible en la captura del panel).

### 6c. Rebuild + instalación del APK con el `projectId` correcto
- **Qué**: el APK que había en el celular de prueba (`versionCode=1`) era anterior al `eas.projectId`, así que nunca pudo registrar un token real. Se hicieron varios builds nuevos (profile `apk`) el 2026-08-31 hasta que el pipeline de EAS Build también se arregló (ver `.easignore` en la raíz — el proyecto subía `app-movil/android/` entero y pasaba el límite de 2GB).
- **Estado**: hecho. El build final (`66fa757c-...`) con `google-services.json` + credencial FCM V1 es el que quedó verificado.
