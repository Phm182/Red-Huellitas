...
const ESPECIES: EspecieCuidado[] = ['perro', 'gato', 'otro'];
...
export default function CuidadosScreen() {
  ...
  const categorias = [...new Set(items.map((c) => c.categoria))] as CategoriaCuidado[];
    ...
    <Atmosphere>
      <ScrollView contentContainerStyle={[styles.contenedor, centeredContent]} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <EmptyState icon="book-outline" titulo={t('cuidados.emptyLista')} />
        ) : (
          categorias.map((cat) => (
...
const styles = StyleSheet.create({
  ...
  chip: { borderWidth: 1, borderRadius: radii.pill, paddingVertical: 8, paddingHorizontal: 16 },
