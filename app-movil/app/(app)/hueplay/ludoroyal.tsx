...
import { DadoSimbolo } from '../../../src/juego/hueludoroyal/DadoSimbolo';
import {
  FichaLudo,
  HuePlaySala,
  JugadaLudoRoyal,
  MovimientoLegalLudo,
  SimboloLudoRoyal,
  TableroLudoRoyal as TableroLudoRoyalEstado,
...
/**
 * HueLudo Real: mismo tablero/fichas/sala que HueLudo clásico
 * (`app/(app)/hueplay/ludo.tsx`, del que esta pantalla es un mirror), pero
 * con dos dados por tirada — numérico (mismo componente `Dado`, ya dibuja
 * puntos reales) y de símbolos (`DadoSimbolo`: Corona/Pluma/en blanco). La
 * Corona libera una ficha del corral, cualquier símbolo o un 6 da turno
 * extra — ver `inc/funciones/ludoroyal.php` para el detalle de reglas.
 */
export default function LudoRoyalScreen() {
  ...
  const [dadoNumerico, setDadoNumerico] = useState<number | null>(null);
  const [simbolo, setSimbolo] = useState<SimboloLudoRoyal | null>(null);
  ...
  const aplicarSala = useCallback((nuevaSala: HuePlaySala) => {
    ...
    if (!animandoRef.current && nuevaSala.tablero !== tableroRef.current) {
      ...
      if (nuevaSala.tablero) {
        const tab: TableroLudoRoyalEstado = JSON.parse(nuevaSala.tablero);
        setPiezas(tab.fichas);
      }
    }
  }, []);
  ...
  const reproducirJugada = useCallback(async (jugada: JugadaLudoRoyal) => {
    if (!jugada.ficha || jugada.desde === null || jugada.hasta === null) {
      return;
    }
    const { jugador, num } = jugada.ficha;
    const desde = jugada.desde === -1 ? -1 : jugada.desde;
    // Si sale del corral (-1), se "aparece" directo en la casilla de entrada
    // (0) y de ahí camina el resto — no hay tramo visual entre el corral y
    // el tablero.
    const inicioAnimacion = desde === -1 ? 0 : desde + 1;
    if (desde === -1) {
      if (!vivoRef.current) return;
      setPiezas((prev) => prev.map((f) => (f.jugador === jugador && f.num === num ? { ...f, pos: 0 } : f)));
      await esperar(MS_POR_PASO);
    }
    for (let p = inicioAnimacion; p <= jugada.hasta; p++) {
      if (!vivoRef.current) return;
  ...
  const reproducirCadenaIA = useCallback(
    async (jugadasIA: { salaJugadorId: number; jugadas: JugadaLudoRoyal[] }[]) => {
  ...
  const tirar = async () => {
    ...
    const res = await hueplayApi.ludoRoyalTirar(sala.salaId);
    if (!vivoRef.current) return;
    setTirando(false);
    ...
    setDadoNumerico(res.data.dadoNumerico);
    setSimbolo(res.data.simbolo);

    if (res.data.pasoElTurno) {
      setAviso(t('hueplay.ludoroyal.sinJugada'));
  ...
  const moverFicha = async (fichaNum: number) => {
    ...
    const res = await hueplayApi.ludoRoyalMover(sala.salaId, fichaNum);
    if (!vivoRef.current) return;
    ...
    >
      ...
      >
        ...
        <Text style={{ color: colors.text, fontSize: 13, flex: 1 }}>
          {terminado
            ? gane
              ? t('hueplay.ludoroyal.ganasteFin')
              : t('hueplay.ludoroyal.perdisteFin')
            : aviso
              ? aviso
              : sala.esMiTurno
                ? movimientosLegales.length > 0
                  ? t('hueplay.ludoroyal.elegiFicha')
                  : t('hueplay.ludoroyal.tocaTirar')
                : t('hueplay.ludoroyal.turnoDe', {
      ...
      <View style={styles.dadoFila}>
        <Dado valor={dadoNumerico} tirando={tirando} color={colorDelTurno} tamano={cell * 1.5} />
        <DadoSimbolo simbolo={simbolo} tirando={tirando} color={colorDelTurno} tamano={cell * 1.5} />
          ...
          >
            {enviando ? (
              ...
              <Text style={{ color: colors.primaryText, fontFamily: fonts.bodySemi, fontSize: 14 }}>
                {t('hueplay.ludoroyal.tirarDados')}
              </Text>
            )}
          </Pressable>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  dadoFila: { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 16 },
