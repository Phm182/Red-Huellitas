...
import { TransitoInteresado } from '../../../../src/types';
import { centeredContent } from '../../../../src/theme/layout';
import { useTheme } from '../../../../src/theme/ThemeProvider';
import { rhMediaUrl } from '../../../../src/utils/media';

/** Molde recortado de adopcion/[id]/postulaciones.tsx, sin bloque de preguntas. */
export default function TransitoInteresadosScreen() {
  const { t } = useTranslation();
  const { colors } = useTheme();
  const { id } = useLocalSearchParams<{ id: string }>();

  const [interesados, setInteresados] = useState<TransitoInteresado[]>([]);
  ...
  useFocusEffect(
    useCallback(() => {
      let activo = true;
      setLoading(true);
      transitoApi.interesadosListar(Number(id)).then((res) => {
  ...
  return (
    <FlatList
      ...
      data={interesados}
      keyExtractor={(i) => String(i.transitoInteresId)}
      ListEmptyComponent={<EmptyState icon="hand-left-outline" titulo={t('transito.emptyInteresados')} />}
      renderItem={({ item }) => (
