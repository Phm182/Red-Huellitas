...
if ((int) $mascota['UserId'] !== $userId) {
    json_error('No tenés permiso para eliminar esta mascota', 403);
}

$stmt = $conn->prepare("UPDATE Mascota SET Estado = 'I' WHERE MascotaId = ?");
$stmt->bind_param('i', $mascotaId);
$stmt->execute();
$stmt->close();

json_success(null, 'Mascota eliminada');
